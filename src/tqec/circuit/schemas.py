...
class ScheduledCircuitModel(BaseModel):
    ...
    def to_scheduled_circuit(self) -> ScheduledCircuit:
        return ScheduledCircuit(
            self.circuit_type.parse(self.circuit_repr), self.schedule
        )

    @staticmethod
    def from_scheduled_circuit(
        scheduled_circuit: ScheduledCircuit, circuit_type: SupportedCircuitTypeEnum
    ) -> ScheduledCircuitModel:
        return ScheduledCircuitModel(
            circuit_type=circuit_type,
            circuit_repr=circuit_type.export(scheduled_circuit.raw_circuit),
            schedule=scheduled_circuit.schedule,
        )
